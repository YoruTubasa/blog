...
const buildNavBarConfig = (): NavBarConfig => {
	...
	if (siteConfig.pages.calendar) {
		...
		myChildren.push({ name: "动态", url: "/dynamic/", icon: "material-symbols:chat-bubble-outline-rounded" });
		myChildren.push({ name: "更新日志", url: "/changelog/", icon: "material-symbols:update" });
	}
